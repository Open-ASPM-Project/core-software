...
interface EditSSOConfigDialogProps {
  onSuccess: () => void;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData: SSOConfigData;
  commonAPIRequest?: <T>(
    requestParams: {
      api: string;
...
const EditSSOConfigDialog: React.FC<EditSSOConfigDialogProps> = ({
  onSuccess,
  open,
  onOpenChange,
  initialData,
  ...
  const updateSSOConfig = React.useCallback(
    async (
      data: SSOConfigData,
      ...
      const endpoint = API_ENDPOINTS.sso.updateConfig;
  ...
  React.useEffect(() => {
    if (initialData) {
      Object.keys(initialData).forEach((key) => {
        formik.setFieldValue(key, initialData[key as keyof SSOConfigData]);
      });
    }
  }, [initialData]);
        ...
        <DialogHeader className="p-6 rounded-t-lg bg-gradient-to-r from-violet-500/10 to-blue-500/10 dark:from-violet-600/20 dark:to-blue-600/20 border-b border-violet-100 dark:border-violet-800/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <DialogDescription>Update Single Sign-On provider configuration.</DialogDescription>
            </div>
          </div>
        </DialogHeader>
...
export default withAPIRequest(EditSSOConfigDialog);
