...
interface AddAssetsAllowListDialogProps {
  open: boolean;
...
const validationSchema = yup.object().shape({
  active: yup.boolean(),
  repos: yup.array().of(yup.number()).min(1, 'At least one repository must be selected'),
  comment: yup.string().max(500, 'Comment must not exceed 500 characters'),
});

const AddAssetsAllowListDialog: React.FC<AddAssetsAllowListDialogProps> = ({
  open,
  onOpenChange,
  commonAPIRequest,
  onSuccess,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        await createAssetsAllowList(payload);

        toast.success('Assets allow list added successfully');
        onOpenChange(false);
        onSuccess();
        handleFormReset();
      } catch (error) {
        toast.error('Failed to create assets allow list');
      } finally {
        setSubmitting(false);
      }
    },
  ...
  const createAssetsAllowList = async (payload: any) => {
      ...
      commonAPIRequest(
        ...
        (response) => {
          if (response) {
            ...
            reject(new Error('Failed to create assets allow list'));
          }
        }
      );
    });
  ...
  // Update formik values when selections change
  React.useEffect(() => {
      ...
      <DialogContent className="sm:max-w-[600px] p-0 gap-0">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <DialogTitle>Add New Assets Allow List</DialogTitle>
              <DialogDescription>
                Create a new assets allow list to manage your security exceptions.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-4">
              ...
              >
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Assets allow list will be enforced when active
                  </p>
                </div>
              </Label>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Add a comment about this assets allow list..."
...
export default withAPIRequest(AddAssetsAllowListDialog);
