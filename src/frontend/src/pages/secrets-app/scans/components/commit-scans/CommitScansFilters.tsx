...
const CommitScansFiltersBase: React.FC<FiltersProps> = ({
  onFiltersChange,
  initialFilters = {},
  commonAPIRequest,
}) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      commonAPIRequest(
        {
          api: `${API_BASE_URLS.development}/live_commits/scan/filters`,
          method: 'GET',
        },
        (response: { filters: FilterField[] } | null) => {
          setLoading(false);
          if (response?.filters) {
      ...
      <DialogContent className="sm:max-w-[500px] h-[600px] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle>Commit Scan Filters</DialogTitle>
          </div>
        </DialogHeader>
...
export const CommitScansFilters = withAPIRequest(CommitScansFiltersBase);
