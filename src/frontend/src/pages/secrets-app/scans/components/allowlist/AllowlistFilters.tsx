...
const AllowlistFiltersBase: React.FC<FiltersProps> = ({
  onFiltersChange,
  initialFilters = {},
  commonAPIRequest,
}) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      commonAPIRequest(
        {
          api: `${API_BASE_URLS.development}/whitelist/filter`,
          method: 'GET',
        },
        (response) => {
          setLoading(false);
      ...
      <DialogContent className="sm:max-w-[500px] h-[600px] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <div className="flex items-center justify-between">
            <DialogTitle>Allowlist Filters</DialogTitle>
          </div>
        </DialogHeader>
...
export const AllowlistFilters = withAPIRequest(AllowlistFiltersBase);
