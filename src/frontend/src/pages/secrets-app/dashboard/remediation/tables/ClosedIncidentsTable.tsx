...
interface Incident {
  ...
  secret?: Secret;
  vulnerability?: any; // Define proper type when needed
}
...
const getSeverityVariant = (severity: string) => {
  const variants = {
    ...
    high: 'bg-orange-500/15 text-orange-600 border-orange-500/20 hover:bg-orange-500/25 dark:text-orange-400',
    medium:
...
function ClosedIncidentsTableComponent({ commonAPIRequest, dateRange }: APIRequestProps) {
  ...
  const columns: ColumnDef<Incident>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-md ${type === 'secret' ? 'bg-orange-500/10' : 'bg-red-500/10'}`}
            >
              {type === 'secret' ? (
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-medium">{row.original.repository.name}</span>
            <span className="text-xs text-muted-foreground">{row.original.repository.author}</span>
          </div>
        </div>
      ),
    },
    {
      accessorKey: 'secret.severity',
      header: ({ column }) => (
        <Button
          variant="ghost"
          className="font-semibold hover:text-primary"
      ...
      cell: ({ row }) => {
        const severity = row.original.secret?.severity || 'unknown';
        return (
    ...
    {
      accessorKey: 'secret.file',
      header: 'Location',
      cell: ({ row }) => (
        <div className="flex flex-col">
          <span className="font-medium">{row.original.secret?.file}</span>
          <span className="text-xs text-muted-foreground">Line: {row.original.secret?.line}</span>
        </div>
      ),
    },
    {
    ...
    //         </DropdownMenuItem>
    //         <DropdownMenuItem>
    //           <LockIcon className="mr-2 h-4 w-4" /> Close Incident
    //         </DropdownMenuItem>
    //       </DropdownMenuContent>
    //     </DropdownMenu>
    //   ),
...
export const ClosedIncidentsTable = withAPIRequest(ClosedIncidentsTableComponent);
