...
interface VMIpAssetTableProps {
  data: PaginatedResponse;
  onPageChange: (page: number) => void;
  isLoading?: boolean;
}

export function VMIpAssetTable({ data, onPageChange, isLoading = false }: VMIpAssetTableProps) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const currentPageAssets =
        data.data?.map((asset) => ({
          uuid: asset.uuid,
          name: asset.name || asset.ipAddress, // Use ipAddress as fallback for name
        })) || [];
  ...
  const getIpTypeBadge = (ipType: string) => {
    const colorMap: Record<string, string> = {
      ipv4: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
      ipv6: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
    };

    return (
      <Badge variant="outline" className={`text-xs font-mono ${colorMap[ipType] || ''}`}>
        {ipType.toUpperCase()}
      </Badge>
    );
  };
                ...
                <TableCell className="p-0 ps-2">
                  <div className="h-16 flex items-center justify-center">
                    <Checkbox
                      ...
                      aria-label={`Select ${asset.name || asset.ipAddress}`}
                    />
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-col gap-0.5">
                    <div className="flex items-center gap-2">
                      <span className="font-medium truncate" title={asset.ipAddress}>
                        {asset.ipAddress}
                      </span>
                      <Button
                        variant="ghost"
                        ...
                        onClick={() => openInNewTab(asset.ipAddress)}
                      >
                        <ExternalLink className="h-3.5 w-3.5 text-muted-foreground" />
                      </Button>
                    </div>
                    {asset.name && asset.name !== asset.ipAddress && (
                      <span className="text-xs text-muted-foreground truncate" title={asset.name}>
                        {asset.name}
                      </span>
                    )}
                  </div>
                </TableCell>
                <TableCell>{getIpTypeBadge(asset.ipType)}</TableCell>
                <TableCell>
                  <span
