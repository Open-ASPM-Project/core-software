...
import { Layout, Shield, Scan, AlertTriangle, FileWarning } from 'lucide-react';

interface PreviewCodeTabsProps {
  className?: string;
}

const VMAppTab = ({ className }: PreviewCodeTabsProps) => {
  ...
  const tabStyle = cn(
    ...
    '[&[data-value=assessments]]:hover:from-emerald-50 [&[data-value=assessments]]:hover:to-blue-50',
    '[&[data-value=assessments]]:data-[state=active]:from-emerald-500/10 [&[data-value=assessments]]:data-[state=active]:to-blue-500/10',
    '[&[data-value=remediation]]:hover:from-red-50 [&[data-value=remediation]]:hover:to-rose-50',
    '[&[data-value=remediation]]:data-[state=active]:from-red-500/10 [&[data-value=remediation]]:data-[state=active]:to-rose-500/10',
    ...
    '[&[data-value=assessments]]:dark:hover:from-emerald-900/20 [&[data-value=assessments]]:dark:hover:to-blue-900/20',
    '[&[data-value=assessments]]:dark:data-[state=active]:from-emerald-400/20 [&[data-value=assessments]]:dark:data-[state=active]:to-blue-400/20',
    '[&[data-value=remediation]]:dark:hover:from-red-900/20 [&[data-value=remediation]]:dark:hover:to-rose-900/20',
    '[&[data-value=remediation]]:dark:data-[state=active]:from-red-400/20 [&[data-value=remediation]]:dark:data-[state=active]:to-rose-400/20',
  ...
  return (
    ...
    >
      <TabsList className="w-full justify-start rounded-xl border bg-gradient-to-r from-slate-50 to-white dark:from-slate-900/50 dark:to-slate-900 shadow-md p-6 space-x-2">
        ...
        <TabsTrigger
          value="assets"
          onClick={() => handleNavigation('vm/assets')}
          className={tabStyle}
        >
          <div
            className={cn(
          ...
          >
            <Shield className={cn(iconBaseStyle, iconStyles.assets)} />
          </div>
          Assets
        </TabsTrigger>
        <TabsTrigger
          value="assessments"
          onClick={() => handleNavigation('vm/assessments')}
          className={tabStyle}
        >
          <div
            className={cn(
          ...
          >
            <Scan className={cn(iconBaseStyle, iconStyles.assessments)} />
          </div>
          Scans
        </TabsTrigger>
        <TabsTrigger
          value="remediation"
          onClick={() => handleNavigation('vm/remediation')}
          className={tabStyle}
        >
          <div
            className={cn(
          ...
          >
            <FileWarning className={cn(iconBaseStyle, iconStyles.remediation)} />
          </div>
          Incidents
        </TabsTrigger>
      </TabsList>
