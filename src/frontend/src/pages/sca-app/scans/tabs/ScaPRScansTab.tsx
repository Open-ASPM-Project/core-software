...
import { ScaPRScansFilters } from '../components/pr-scans/ScaPRScansFilters';
import ScaPRScansTable from '../tables/pr-scans/ScaPRScansTable';
...
const ScaPRScansTab: React.FC<PRScansTabProps> = ({ commonAPIRequest }) => {
  const queryClient = new QueryClient();
  const [searchParams, setSearchParams] = useSearchParams();
  const [isLoading, setIsLoading] = React.useState(false);
  const [prScans, setPRScans] = React.useState<PaginatedResponse | null>(null);
  const [activeFilters, setActiveFilters] = React.useState<Record<string, string[]>>({});

  const page = Number(searchParams.get('page')) || 1;
  const limit = Number(searchParams.get('limit')) || 10;
  const search = searchParams.get('search') || '';
  const order_by = (searchParams.get('order_by') as 'asc' | 'desc') || 'asc';
  const sort_by = searchParams.get('sort_by') as QueryParams['sort_by'];

  // Fetch PR Scans
  const fetchPRScans = useCallback(
    (params: QueryParams) => {
      setIsLoading(true);
      const endpoint = API_ENDPOINTS.prScans.getPRScans;
      ...
      // Add pr_id filter
      if (activeFilters.pr_id?.length) {
        activeFilters.pr_id.forEach((pr) => {
          queryParts.push(`pr_id=${pr.id}`);
        });
      }

      // Add status filter
      if (activeFilters.status !== undefined) {
        queryParts.push(`status=${activeFilters.status}`);
      }
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-muted-foreground">
                Showing {((prScans?.current_page || 1) - 1) * (prScans?.current_limit || 10) + 1} to{' '}
                {Math.min(
                  (prScans?.current_page || 1) * (prScans?.current_limit || 10),
                  prScans?.total_count || 0
                )}{' '}
                of {prScans?.total_count || 0}
              </p>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {getPageNumbers(page, prScans?.total_pages || 1).map((pageNum) => (
                <Button
                  key={pageNum}
              ...
              <Button
                ...
                size="sm"
                onClick={() => updateParams({ page: page + 1 })}
                disabled={page === (prScans?.total_pages || 1)}
                className="w-8"
              >
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => updateParams({ page: prScans?.total_pages || 1 })}
                disabled={page === (prScans?.total_pages || 1)}
                className="w-8"
              >
...
export default withAPIRequest(ScaPRScansTab);
