...
// import { ScaAllowlistFilters } from '../components/allowlist/ScaAllowlistFilters';
import AddScaAllowListDialog from '../dialogs/sca-allowlist/AddScaAllowListDialog';
import ScaAllowListTable from '../tables/sca-allowlist/ScaAllowListTable';
import { AllowlistFilters } from '@/pages/secrets-app/scans/components/allowlist/AllowlistFilters';
import ViewAssetAllowlistDialog from '@/pages/secrets-app/scans/dialogs/allowlist/ViewAssetAllowlistDialog';

// Types are same as AllowListTab
interface VCS {
  id: number;
  name: string;
}
...
interface ScaAllowListTabProps {
  commonAPIRequest: <T>(
    requestParams: {
      api: string;
...
const ScaAllowListTab: React.FC<ScaAllowListTabProps> = ({ commonAPIRequest }) => {
  ...
  const fetchAllowList = useCallback(
    (params: QueryParams) => {
      ...
      const queryParts: string[] = [
        `page=${params.page}`,
        `limit=${params.limit}`,
        'type=VULNERABILITY', // Changed to VULNERABILITY
      ];
  ...
  // Rest of the component remains the same as AllowListTab, just change component names
        ...
        <CardContent className="p-0 border-none shadow-none">
          <div className="flex items-center justify-between gap-4 mb-6 mt-4">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search SCA allow list..."
                defaultValue={search}
...
export default withAPIRequest(ScaAllowListTab);
