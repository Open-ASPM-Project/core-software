...
import RepoPropertiesDialog from '@/pages/secrets-app/asset/dialogs/repository/RepoPropertiesDialog';
import AddToNewGroupDialog from '@/pages/secrets-app/asset/dialogs/repository/AddToNewGroupDialog';
import AddToExistingGroupDialog from '@/pages/secrets-app/asset/dialogs/repository/AddToExistingGroupDialog';
import ViewVulnerabilitiesForRepoDialog from '../../dialogs/repository/ViewVulnerabilitiesForRepoDialog';
import RepositoryDetailDialog from '@/pages/secrets-app/asset/dialogs/repository/RepositoryDetailDialog';
import ViewRepoSBOMDialog from '@/pages/secrets-app/asset/dialogs/repository/ViewRepoSBOMDialog';
import AddAllowlistFromRepoDialog from '@/pages/secrets-app/asset/dialogs/repository/AddAllowlistFromRepoDialog';
...
interface QueryParams {
  ...
  search?: string;
  order_by?: 'asc' | 'desc';
  sort_by?: 'vulnerability_count' | 'created_at' | 'score_normalized' | 'author';
}
...
const ScaRepoTable = ({
  ...
  const startRepoScan = async (repoId: number) => {
    return new Promise((resolve, reject) => {
      commonAPIRequest(
        {
          api: `${API_BASE_URLS.development}/vulnerabilities/scan/repo/${repoId}`,
          method: 'POST',
          data: {
  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gradient-to-r from-violet-50 to-blue-50 dark:from-violet-950 dark:to-blue-950">
              ...
              <TableHead>
                <Button
                  onClick={() => handleSortByChange('vulnerability_count')}
                  variant="ghost"
                  className="h-8 w-full justify-start font-bold text-primary hover:bg-purple-50 dark:hover:bg-purple-900/20 hover:text-purple-700 dark:hover:text-purple-300 transition-colors group"
                >
              ...
              repositories?.map((repo) => (
                <TableRow key={repo.id} className="group hover:bg-muted/50 transition-colors">
                  ...
                  <TableCell>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <Bug className="h-4 w-4 text-primary-500 group-hover/btn:scale-110 transition-transform duration-200" />
                        <span className="font-medium">{repo.vulnerability_count} Found</span>
                      </div>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem
                          onClick={() => {
                            navigate(`/sca/incidents?repo_id=${repo?.id}`);
                          }}
                          className="flex items-center gap-2"
                        >
      ...
      {isOpen && (
        <ViewVulnerabilitiesForRepoDialog
          isOpen={isOpen}
