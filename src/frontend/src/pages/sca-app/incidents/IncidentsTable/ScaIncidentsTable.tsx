...
const ScaIncidentsTable = ({
  ...
  const handleViewVulnerabilityDetails = (vulnerability: Vulnerability) => {
    setSelectedVulnId(vulnerability.id); // assuming vulnerability has an id field
    setDialogOpen(true);
  };
      ...
      <TableCell colSpan={7}>
        <div className="flex flex-col items-center justify-center py-16">
          <div className="relative mb-4">
            <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 dark:from-emerald-500/5 dark:to-cyan-500/5 blur-2xl rounded-full" />
            <div className="relative bg-gradient-to-r from-emerald-50 to-cyan-50 dark:from-emerald-900/30 dark:to-cyan-900/30 p-6 rounded-full">
              <Package className="w-12 h-12 text-emerald-600/90 dark:text-emerald-400/90" />
            </div>
          </div>
          <h3 className="text-xl font-semibold bg-gradient-to-r from-emerald-600 to-cyan-600 dark:from-emerald-400 dark:to-cyan-400 bg-clip-text text-transparent mb-3">
            No Vulnerabilities Found
          </h3>
          <p className="text-sm text-muted-foreground text-center max-w-md mb-8">
            No package vulnerabilities have been detected yet. This could mean your dependencies are
            secure, or you might need to run more scans.
          </p>
        </div>
      </TableCell>
    </TableRow>
  ...
  return (
    <>
          ...
          <div className="p-4 border rounded-lg bg-muted/30">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium">Selected Vulnerabilities</span>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="secondary"
                      size="sm"
                      className="h-8 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800"
                    >
      ...
      <div className="rounded-lg border bg-card shadow-sm overflow-hidden custom-scrollbar">
        <Table>
          <TableHeader>
            <TableRow className="bg-gradient-to-r from-emerald-50 to-cyan-50 dark:from-emerald-950 dark:to-cyan-950">
              ...
              <TableHead>
                <Button
                  ...
                  variant="ghost"
                  className="h-8 w-full justify-start font-bold text-primary hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors group"
                >
                  <Package className="mr-2 h-4 w-4 text-emerald-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                  Vulnerability
              ...
              <TableHead>
                <Button
                  ...
                  className="h-8 w-full justify-start font-bold text-primary hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors group"
                >
                  <GitFork className="mr-2 h-4 w-4 text-emerald-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                  Repository
                  <ArrowUpDown className="ml-2 h-4 w-4 opacity-50 group-hover:opacity-70" />
                </Button>
              </TableHead>
              ...
              <TableHead>
                <Button
                  ...
                  className="h-8 w-full justify-start font-bold text-primary hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors group"
                >
                  <AlertTriangle className="mr-2 h-4 w-4 text-emerald-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                  Severity
                  <ArrowUpDown className="ml-2 h-4 w-4 opacity-50 group-hover:opacity-70" />
                </Button>
              </TableHead>
              <TableHead>
                <Button
                  ...
                  className="h-8 w-full justify-start font-bold text-primary hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors group"
                >
                  <Calendar className="mr-2 h-4 w-4 text-emerald-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                  Detected
                  <ArrowUpDown className="ml-2 h-4 w-4 opacity-50 group-hover:opacity-70" />
                </Button>
              </TableHead>
            ...
            {isLoading ? (
              ...
              incidents.map((incident) => (
                <TableRow key={incident.id} className="group hover:bg-muted/50">
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      <h4 className="font-medium text-sm">
                        {incident.vulnerability.cve_id || incident.name}
                        {incident.vulnerability.fix_available && (
                          <Badge
                            variant="outline"
                            className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2 py-0.5"
                          >
                      ...
                      <div className="flex items-center gap-2">
                        {(() => {
                          const {
                            ...
                            icon: Icon,
                            label,
                          } = getPackageTypeBadge(incident.vulnerability.artifact_type);
                          return (
                  ...
                  <TableCell>
                    {incident.vulnerability.repository && (
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="font-medium">
                            {incident.vulnerability.repository.name}
                          </span>
                          <a
                            href={incident.vulnerability.repository.repoUrl}
                            target="_blank"
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        <span className="font-medium">{incident.vulnerability.package}</span>
                  ...
                  <TableCell>
                    {(() => {
                      const {
                        ...
                        icon: Icon,
                        label,
                      } = getSeverityBadge(incident.vulnerability.severity);
                      return (
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem
                          onClick={() => handleViewVulnerabilityDetails(incident.vulnerability)}
                          className="flex items-center gap-2"
                        >
                        ...
                        {incident.vulnerability.repository && (
                          <DropdownMenuItem
                            onClick={() =>
                              handleOpenRepoDialog(incident.vulnerability.repository!.id)
                            }
                            className="flex items-center gap-2"
                          >
      ...
      {selectedIncidentId &&
        incidents.find((i) => i.id === selectedIncidentId)?.vulnerability?.repository && (
...
export default withAPIRequest(ScaIncidentsTable);
