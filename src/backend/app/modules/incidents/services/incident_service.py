...
from app.modules.vunerability_v2.vulnerability_v2_model import VulnerabilityVM
    ...
    if incident_type == IncidentTypeEnum.secret:
        ...
        for sev_str, count in raw_counts:
            try:
                ...
                continue
    elif incident_type == IncidentTypeEnum.vulnerability_vm:
        # Query for vulnerability incidents using the Vulnerability table (severity stored as string)
        query = (
            select(VulnerabilityVM.info_severity, func.count(VulnerabilityVM.id))
            .join(Incidents, VulnerabilityVM.id == Incidents.vulnerability_id)
            .where(
                Incidents.type == incident_type,
                Incidents.status.in_(statuses)
            )
            .group_by(VulnerabilityVM.severity)
        )
        result = await db.execute(query)
        raw_counts = result.all()
        severity_counts = {}
...
async def get_incidents(
    ...
    # Resolve repository IDs from group_ids if provided.
    if group_ids:
        group_repo_query = (
            select(Repo.id)
            .join(Group.repos)
    ...
    if incident_type == IncidentTypeEnum.secret:
        ...
        # For vulnerability_vm incidents
        query = (
            select(Incidents)
            .outerjoin(VulnerabilityVM, Incidents.vulnerability_vm_id == VulnerabilityVM.id)
            .options(
                selectinload(Incidents.vulnerability_vm),
            )
            .where(Incidents.type == incident_type)
        )
    ...
    # Vulnerability-VM filters
    elif incident_type == IncidentTypeEnum.vulnerability_vm:
        if filters.template_ids:
            filters_to_apply.append(VulnerabilityVM.template_id.in_(filters.template_ids))
        if filters.template_paths:
            filters_to_apply.append(VulnerabilityVM.template_path.in_(filters.template_paths))
        if filters.template_encoded:
            filters_to_apply.append(VulnerabilityVM.template_encoded.in_(filters.template_encoded))
        if filters.info_names:
            filters_to_apply.append(VulnerabilityVM.info_name.in_(filters.info_names))
        if filters.severities:
            filters_to_apply.append(VulnerabilityVM.info_severity.in_(filters.info_severities))
        if filters.info_descriptions:
            filters_to_apply.append(VulnerabilityVM.info_description.in_(filters.info_descriptions))
        if filters.info_impacts:
            filters_to_apply.append(VulnerabilityVM.info_impact.in_(filters.info_impacts))
        if filters.info_remediations:
            filters_to_apply.append(VulnerabilityVM.info_remediation.in_(filters.info_remediations))
        if filters.types:
            filters_to_apply.append(VulnerabilityVM.type.in_(filters.types))
        if filters.hosts:
            filters_to_apply.append(VulnerabilityVM.host.in_(filters.hosts))
        if filters.ports:
            filters_to_apply.append(VulnerabilityVM.port.in_(filters.ports))
        if filters.schemes:
            filters_to_apply.append(VulnerabilityVM.scheme.in_(filters.schemes))
        if filters.urls:
            filters_to_apply.append(VulnerabilityVM.url.in_(filters.urls))
        if filters.matched_ats:
            filters_to_apply.append(VulnerabilityVM.matched_at.in_(filters.matched_ats))
        if filters.requests:
            filters_to_apply.append(VulnerabilityVM.request.in_(filters.requests))
        if filters.responses:
            filters_to_apply.append(VulnerabilityVM.response.in_(filters.responses))
        if filters.ips:
            filters_to_apply.append(VulnerabilityVM.ip.in_(filters.ips))
        if filters.curl_commands:
            filters_to_apply.append(VulnerabilityVM.curl_command.in_(filters.curl_commands))
        if filters.matcher_status is not None:
            filters_to_apply.append(VulnerabilityVM.matcher_status == filters.matcher_status)
        if filters.profiles:
            filters_to_apply.append(VulnerabilityVM.profile.in_(filters.profiles))
        if filters.asset_ids:
            filters_to_apply.append(VulnerabilityVM.asset_id.in_(filters.asset_ids))
        if filters.asset_names:
            filters_to_apply.append(VulnerabilityVM.asset_name.in_(filters.asset_names))
        if filters.asset_types:
            filters_to_apply.append(VulnerabilityVM.asset_type.in_(filters.asset_types))
        # Global search example for vulnerability_vm
        if filters.search:
            filters_to_apply.append(
                or_(
                    VulnerabilityVM.template_id.ilike(f"%{filters.search}%"),
                    VulnerabilityVM.info_description.ilike(f"%{filters.search}%"),
                    VulnerabilityVM.host.ilike(f"%{filters.search}%"),
                    VulnerabilityVM.url.ilike(f"%{filters.search}%"),
                )
            )
    ...
    response_data = [
        {
            ...
            "vulnerability_vm": incident.vulnerability_vm,
            "repository": {
...
async def get_filter_values(
    ...
    # Map incoming filter keys to actual column names.
    FILTER_COLUMN_MAP = {
        'statuses': 'status',
        'severities': 'severity',
        ...
        # Vulnerability-VM-specific
        'template_ids': 'template_id',
        'template_paths': 'template_path',
        'template_encoded': 'template_encoded',
        'info_names': 'info_name',
        'info_descriptions': 'info_description',
        'info_impacts': 'info_impact',
        'info_remediations': 'info_remediation',
        'types': 'type',
        'hosts': 'host',
        'ports': 'port',
        'schemes': 'scheme',
        'urls': 'url',
        'matched_ats': 'matched_at',
        'requests': 'request',
        'responses': 'response',
        'ips': 'ip',
    ...
    if filter_name in ["status", "type"]:
        ...
        if incident_type == "vulnerability":
            model = Vulnerability
        elif incident_type == "vulnerability_vm":
            model = VulnerabilityVM
        else:
            model = Secrets
