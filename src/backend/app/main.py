    ...
    async for db in get_db():
        try:
            if isinstance(db, AsyncSession):
                ...
                scheduler.add_job(
                    calculate_score, CronTrigger(minute="*/30"), args=[db]
                )
                scheduler.add_job(
                    sca_whitelist_fix_cron, CronTrigger(hour="*/3"), args=[db]
                )
                scheduler.add_job(
                    validate_license_cron, CronTrigger(minute="*/10"), args=[db]
                )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "app.main:app", host="0.0.0.0", port=settings.PORT, reload=settings.RELOAD
    )  # Set port to 3000
